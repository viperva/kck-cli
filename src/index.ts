...
loop: while (true) {
  ...
  if (!isConfig) {
    ...
    degrees.forEach((d) => {
      if ([1, 5, 6, 8, 14, 15, 16, 19, 20].includes(d.id)) return;
      degreeChoices.push({
        title: d.name,
        value: d.id,
      });
  ...
  const tableConfig = {
    ...
    spanningCells: new Array<colSpan>(),
  };
  ...
  schedules.forEach((lesson) => {
    ...
    if (lessonSubject) {
      ...
      } else nameString = chalk.black.bgCyanBright(`\n${nameString}\n`);

      hours[lessonStart as keyof typeof hours][lesson.day] = nameString;
    }
  });
